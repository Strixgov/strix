/**
 * AC-1 canonical bytes — the single byte contract.
 *
 ...
 * byte-equivalence this module guarantees is proven against the AC-1
 * conformance corpus (`vectors/transaction_proof_v1/`) — every positive
 * vector's `canonical_bytes_hex` is reproduced exactly.
 *
 * The serializer itself is `canonicalizeJSON` from the SCJ v1 mirror —
 * the same chokepoint AA-1 signs/verifies through. There is exactly one
 * canonicalizer in this SDK; AC-1 does not introduce a parallel shape.
 */
